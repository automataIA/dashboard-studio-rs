use crate::features::dashboard::{
    config::builders::PieConfig, config::style::PieStyleOptions,
    config::traits::WidgetConfigBuilder, DashboardContext,
};
use crate::ui::molecules::EChartsWrapper;
use leptos::prelude::*;

/// PieChart Widget with ECharts
///
/// Supports pie, donut, and rose chart variants.
/// Uses real data from active dataset with configurable field mapping.
///
/// # Features
/// - Real data from CSV datasets
/// - Configurable field mapping (labels, values)
/// - Style options (inner radius, rose chart, border radius, etc.)
...
/// ```rust
/// view! {
///     <PieChartWidget widget_id="widget_pie_chart" />
/// }
/// ```
#[component]
pub fn PieChartWidget(
    ...
    // Generate ECharts options using PieConfig builder
    let options = Memo::new(move |_| {
        widget.get().and_then(|w| {
            ...
            let style_options: PieStyleOptions =
                serde_json::from_str(&w.chart_config.style_options)
                    .unwrap_or_else(|_| PieStyleOptions::default());

            // Build ECharts options using PieConfig
            PieConfig
